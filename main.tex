...
For foreground predictions, we evaluate the correlation between predicted and true foregrounds in harmonic space through their binned cross-power spectrum: $C_\ell^{\hat{F}_L \times F_L} = \frac{1}{2\ell+1} \sum_{m=-\ell}^{\ell} \hat{a}_{\ell m}^{\hat{F}_L} (a_{\ell m}^{F_L})^*$,
where $\hat{a}_{\ell m}^{\hat{F}_L}$ and $a_{\ell m}^{F_L}$ are the spherical harmonic coefficients of the predicted and true foreground maps, respectively. The resulting cross-spectrum is binned with bin width of $\Delta \ell = 50$. Similar to the spatial analysis, we compute the distribution of \textit{null} cross-power spectra between each prediction and all target maps to assess statistical significance.

For CMB reconstructions, we use the cross-power spectrum between the reconstructed and true CMB maps to verify the unbiasedness condition given in Equation~\eqref{eq:unbiasedness_condition}. Specifically, we compute the normalized cross-power spectrum $C_\ell^{\hat{S} \times S} / C_\ell^{S \times S}$ and verify that it approaches unity across all multipoles, which demonstrates that the reconstruction preserves the cosmological signal without bias. This validation is critical for ensuring that the signal-preserving framework is functioning correctly and that subsequent cosmological inference remains unbiased.
